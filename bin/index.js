...
import { runSentimentAnalysisWorkflow } from '../src/sentimentAnalyzer.js';
...
// Helper to generate the ASCII face/wave logo
function getAsciiLogo() {
  ...
  return [
    ...
    magenta("       ~ ~ ~"),
    magenta("     ~       ~"),
    magenta("    (  *   *  )") + purple("    ai-asset-sentiment"),
    magenta("     ~   ^   ~") + purple("     =================="),
...
program
  .name('ai-asset-sentiment')
  .description('Google Ads Asset Sentiment & Big Five Psychometrics CLI')
  .version('1.0.0');
...
program
  ...
  .description('Run sentiment analysis workflow to score assets and generate SUPER ADS')
  .option('-s, --sandbox', 'Force Sandbox mode with simulated campaigns data')
  .action(async (options) => {
    console.log(chalk.bold.magenta('\n=== Run Asset Sentiment Analysis Pipeline ===\n'));
    ...
    try {
      const results = await runSentimentAnalysisWorkflow(config, accessToken, isSandbox);

      console.log(chalk.bold.magenta('\n=== Target Audience Fit & SUPER ADS Report ===\n'));
...
async function showInteractiveDashboard() {
  ...
  console.log(`Active Agents:           ${chalk.magenta('sentiment_agent, super_ad_agent')}`);
  console.log(chalk.gray('---------------------------\n'));

  const choice = await select({
    message: 'Select action to execute:',
    choices: [
      { name: '1. Run Asset Sentiment & Big Five Analysis (run-workflow)', value: 'run-workflow' },
